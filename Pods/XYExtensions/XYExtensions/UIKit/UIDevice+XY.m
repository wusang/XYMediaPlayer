...
+ (NSString *)xy_platformString{
    NSString *platform = [self xy_platform];
...
+ (NSUInteger)xy_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)xy_cpuFrequency {
    return [self xy_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)xy_busFrequency {
    return [self xy_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)xy_ramSize {
    return [self xy_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)xy_cpuNumber {
    return [self xy_getSysInfo:HW_NCPU];
}


+ (NSUInteger)xy_totalMemoryBytes
{
    return [self xy_getSysInfo:HW_PHYSMEM];
}
