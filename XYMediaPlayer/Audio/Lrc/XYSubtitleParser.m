...
@implementation XYSubtitleParser
+ (XYSubtitleParser *)parser{
    static XYSubtitleParser * macro = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        macro = [[XYSubtitleParser alloc]init];
    });
    return macro;
}
...
- (NSDictionary *)parseLrc:(NSString *)lrc{
    ...
    lrc = [lrc stringByReplacingOccurrencesOfString:@"[" withString:[[NSString xy_Char1] stringByAppendingString:@"\n"]];
    ...
    for (NSString *s in singlearray) {
        ...
        if ([str isEqualToString:[NSString xy_Char1]]) {
            j = 0;
        }
        ...
        switch (j) {
            ...
            case 2:
            {
                ...
                if ([nextStr isEqualToString:[NSString xy_Char1]]) {
    ...
    for (int i = 0; i < begintimearray.count; i++) {
        ...
        [dic setObject:[begintimearray xy_objectAtIndex:i] forKey:@"beginTime"];
        [dic setObject:[subtitlesarray xy_objectAtIndex:i] forKey:@"subtitles"];
        [arr addObject:dic];
    }
...
- (NSDictionary *)parseSrt:(NSString *)srt{
    ...
    for (int i = 0; i < begintimearray.count; i++) {
        NSMutableDictionary *dic = [NSMutableDictionary dictionary];
        [dic setObject:[begintimearray xy_objectAtIndex:i] forKey:@"beginTime"];
        [dic setObject:[endtimearray xy_objectAtIndex:i] forKey:@"endTime"];
        [dic setObject:[subtitlesarray xy_objectAtIndex:i] forKey:@"subtitles"];
        [arr addObject:dic];
    }
    NSMutableDictionary *info = [NSMutableDictionary dictionary];
