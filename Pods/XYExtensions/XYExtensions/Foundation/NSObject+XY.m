...
- (NSArray*)xy_propertyKeys{
    return [[self class] xy_propertyKeys];
}
+ (NSArray *)xy_propertyKeys {
...
- (NSArray *)xy_propertiesInfo{
    return [[self class] xy_propertiesInfo];
}
...
+ (NSArray *)xy_propertiesInfo{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self xy_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)xy_propertiesWithCodeFormat{
    ...
    NSArray *properties = [[self class] xy_propertiesInfo];
...
-(NSArray*)xy_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] xy_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] xy_decodeType:returnType];
        NSString *encodeString = [[self class] xy_decodeType:encoding];
...
-(NSDictionary *)xy_protocolList{
    return [[self class]xy_protocolList];
}
+ (NSDictionary *)xy_protocolList{
...
+ (NSArray *)xy_instanceVariable{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] xy_decodeType:ivar_getTypeEncoding(ivars[i])];
...
+ (NSDictionary *)xy_dictionaryWithProperty:(objc_property_t)property{
...
+ (NSString *)xy_decodeType:(const char *)cString{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString xy_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
